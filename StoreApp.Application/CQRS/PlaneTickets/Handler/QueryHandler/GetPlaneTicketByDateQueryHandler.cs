...
public class GetPlaneTicketByDateQueryHandler : IRequestHandler<GetPlaneTicketByDateQueryRequest, Pagination<GetAllPlaneTicketQueryResponse>>
{
    private readonly IUnitOfWork _unitOfWork;
    public GetPlaneTicketByDateQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<Pagination<GetAllPlaneTicketQueryResponse>> Handle(GetPlaneTicketByDateQueryRequest request, CancellationToken cancellationToken)
    {
        var query = _unitOfWork.PlaneTicketRepository.GetAll()
            .Where(pt => pt.DueDate.Date == request.Date.Date);
