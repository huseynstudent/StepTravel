...
public class ReturnPlaneTicketCommandHandler
    : IRequestHandler<ReturnPlaneTicketCommandRequest, ResponseModel<ReturnPlaneTicketCommandResponse>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly StoreAppDbContext _db;
    private readonly IConfiguration _configuration;
    private readonly ILogger<ReturnPlaneTicketCommandHandler> _logger;

    public ReturnPlaneTicketCommandHandler(
        IUnitOfWork unitOfWork,
        StoreAppDbContext db,
        IConfiguration configuration,
        ILogger<ReturnPlaneTicketCommandHandler> logger)
    {
    ...
    public async Task<ResponseModel<ReturnPlaneTicketCommandResponse>> Handle(
        ReturnPlaneTicketCommandRequest request,
        CancellationToken cancellationToken)
    {
        var ticket = await _unitOfWork.PlaneTicketRepository.GetByIdAsync(request.Id);
        if (ticket == null || ticket.IsDeleted)
            return new ResponseModel<ReturnPlaneTicketCommandResponse>(null);
            ...
            _ = Task.Run(() =>
            {
                try
                {
                    ...
                    emailService.Send(
                        ...
                        appPass,
                        user.Email,
                        "✈ Ticket Return Confirmation – StepTravel",
                        $@"
                    ...
                    _logger.LogInformation("Refund email sent to {Email}", user.Email);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to send refund email to {Email}", user.Email);
                }
            });
        }

        return new ResponseModel<ReturnPlaneTicketCommandResponse>(new ReturnPlaneTicketCommandResponse
        {
            Id = ticket.Id,
